...
uk = Map(location=[51.863976, 0.835467], zoom_start=10, control_scale=True)
...
marker_cluster = FastMarkerCluster(data).add_to(uk)
...
for each in toilets.iterrows():  
    ...
    Marker(list(OSGB36toWGS84(each[1]['Geo X'],each[1]['Geo Y'])), popup=each[1]['Street Address']).add_to(marker_cluster)
 
# we can also add a marker directly to the map, outside of the clustering
Marker([51.863976, 0.835467], popup="Local Pub not included in marker cluster",icon=Icon(color='red',icon='info-sign')).add_to(uk)
